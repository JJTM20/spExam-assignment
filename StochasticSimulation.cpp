...
//Prototypes
    ...
    void StochasticSimulation::RunSimulation(Vessel vessel, double end_time) {
        std::string mCount;
        std::string header;
        auto vName = vessel.GetName();
        this->path = "..\\out\\trajectory_" + cleanString(vName) + ".csv";
        ...
        while (vessel.global_state.GetCurrentTime() <= end_time) {
            ...
            bool valid_reaction = std::all_of(min_delay_reaction.get_reactants().begin(), min_delay_reaction.get_reactants().end(),
                                              [&](Molecule &i) {
    ...
    void StochasticSimulation::RunSimulationParallel(Vessel vessel, double end_time, int numberOfSims, Obs observer) {
        ...
        for (int i = 0; i < numberOfSims; ++i) {
            threads.emplace_back(&StochasticSimulation::RunSimulation, &simulations[i], std::ref(vessel), end_time, observer);
        }
